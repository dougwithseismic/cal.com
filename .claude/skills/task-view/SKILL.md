...
5. Show available actions:
   - `/task-start <id>` — if status is todo
   - `/task-done <id>` — if status is in-progress
   - `/task-update <id> status blocked` — if stuck
   - `/task-update <id> notes "..."` — to add notes

6. If the task is not found, show an error and suggest `/task-list all`.
