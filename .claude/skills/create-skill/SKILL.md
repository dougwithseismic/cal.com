...
Skills can include supporting files in their directory. This is the recommended approach for reducing token usage — put structure in a template file, and have the skill read + fill it in rather than generating from scratch.
...
This pattern is especially powerful for skills that produce structured output (knowledge base entries, PR descriptions, changelogs, etc.) — the template carries the structure so Claude just fills in the data.
