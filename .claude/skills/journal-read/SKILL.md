...
6. If the database does not exist, inform the user and suggest running the init script:
   ```bash
   CLAUDE_PROJECT_DIR="$CLAUDE_PROJECT_DIR" "$CLAUDE_PROJECT_DIR/.claude/hooks/init-journal-db.sh"
   ```
