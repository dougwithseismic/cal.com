...
```bash
node "$CLAUDE_PROJECT_DIR/.claude/scripts/sync-issues.js" $ARGUMENTS
```
...
| **Read** | Use `/issues-list` and `/issues-view` to query local DB |
| **Update** | Re-sync updates GitHub fields; `/issues-triage` updates local metadata |
