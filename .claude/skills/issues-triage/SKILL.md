...
2. Actions:
   ...
   | Action | Usage | Effect |
   |--------|-------|--------|
   | `priority` | `/issues-triage 28283 priority high` | Sets priority to: critical, high, medium, low, unset |
   | `status` | `/issues-triage 28283 status in-progress` | Sets tracking_status to: new, triaged, in-progress, resolved, ignored |
   | `note` | `/issues-triage 28283 note Needs repro steps` | Appends to the notes field |
   | `ignore` | `/issues-triage 28283 ignore` | Shortcut: sets tracking_status to 'ignored' |
   | `pick` | `/issues-triage 28283 pick` | Shortcut: sets tracking_status to 'in-progress' |
...
5. Validate:
   ...
   - Issue must exist in the DB (suggest `/issues-sync` if not found)
