 ...
 *
 * Environment:
 *   DB_PATH - path to journal.db (defaults to .claude/journal/journal.db relative to script)
 */
...
const DB_PATH =
  process.env.DB_PATH ||
  path.resolve(__dirname, "..", "journal", "journal.db");
...
function fetchIssues(stateFlag) {
  ...
  try {
    ...
    console.error(`Failed to fetch ${stateFlag} issues from GitHub. Is gh authenticated?`);
...
console.log(`Synced ${issues.length} issues (${openCount} open, ${closedCount} closed)`);
  ...
  for (const issue of issues) {
    ...
    if (!before) {
      ...
      if (before.state !== issue.state) {
        stateChanges.push(`  #${num} ${before.state} -> ${issue.state}: ${issue.title}`);
